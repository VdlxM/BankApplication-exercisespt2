...
public class BankAccountTest {
	...
	// ========== TESTS EDGE CASES POUR DEPOSIT ==========

	@Test
	public void test_deposit_zero_amount() {
		...
		// Then: le solde ne change pas
		assertEquals("Le dépôt de 0 ne devrait pas changer le solde",
					 initialBalance, bankAccount.getBalance(), 0.01);
	}
	...
	public void test_deposit_negative_amount() {
		...
		// When: on essaie de déposer un montant négatif
		bankAccount.depositMoney(-100.0);

		// Then: le solde ne devrait pas changer
		assertEquals("Le dépôt négatif ne devrait pas être accepté",
					 initialBalance, bankAccount.getBalance(), 0.01);
	}
	...
	public void test_deposit_large_amount() {
		// Given: un compte avec un solde initial
		bank.addAccount(bankAccount, assignAccountNumber);
		assertEquals(5000, bankAccount.getBalance(), 0.01);

		// When: on dépose un très grand montant
		bankAccount.depositMoney(1000000.0);

		// Then: le solde devrait augmenter correctement
		assertEquals("Le dépôt d'un grand montant devrait fonctionner",
					 1005000.0, bankAccount.getBalance(), 0.01);
	}

	// ========== TESTS EDGE CASES POUR WITHDRAW ==========

	@Test
	public void test_withdraw_exact_balance() {
		// Given: un compte avec 5000
		bank.addAccount(bankAccount, assignAccountNumber);
		assertEquals(5000, bankAccount.getBalance(), 0.01);

		// When: on essaie de retirer exactement le solde (mais > limite de 700)
		boolean result = bankAccount.withdrawMoney(5000.0);

		// Then: le retrait échoue car il dépasse la limite journalière
		assertFalse("Le retrait ne devrait pas réussir (dépasse la limite)", result);
		assertEquals(5000, bankAccount.getBalance(), 0.01);
	}
	...
	public void test_withdraw_zero_amount() {
		...
		// When: on essaie de retirer 0
		boolean result = bankAccount.withdrawMoney(0.0);

		// Then: le retrait échoue (retirer 0 n'a pas de sens)
		assertFalse("Le retrait de 0 devrait échouer", result);
		assertEquals(initialBalance, bankAccount.getBalance(), 0.01);
		assertEquals(0, bankAccount.getAmountWithdrawn(), 0.01);
	}
	...
	public void test_withdraw_negative_amount() {
		...
		// When: on essaie de retirer un montant négatif
		boolean result = bankAccount.withdrawMoney(-100.0);

		// Then: le retrait échoue
		assertFalse("Le retrait négatif devrait échouer", result);
		assertEquals(initialBalance, bankAccount.getBalance(), 0.01);
		assertEquals(0, bankAccount.getAmountWithdrawn(), 0.01);
	}

	@Test
	public void test_withdraw_exceeds_balance() {
		// Given: un compte avec 5000
		bank.addAccount(bankAccount, assignAccountNumber);
		assertEquals(5000, bankAccount.getBalance(), 0.01);

		// When: on essaie de retirer plus que le solde disponible
		boolean result = bankAccount.withdrawMoney(6000.0);

		// Then: le retrait échoue
		assertFalse("Le retrait ne devrait pas réussir (solde insuffisant)", result);
		assertEquals(5000, bankAccount.getBalance(), 0.01);
		assertEquals(0, bankAccount.getAmountWithdrawn(), 0.01);
	}

	@Test
	public void test_withdraw_exceeds_single_transaction_limit() {
		// Given: un compte avec 5000 et limite de 700
		bank.addAccount(bankAccount, assignAccountNumber);
		assertEquals(700, bankAccount.getWithdrawLimit(), 0.01);

		// When: on essaie de retirer plus que la limite en une seule fois
		boolean result = bankAccount.withdrawMoney(800.0);
	...
	@Test
	public void test_withdraw_cumulative_exceeds_daily_limit() {
		// Given: un compte avec limite de 700
		bank.addAccount(bankAccount, assignAccountNumber);

		// When: on retire plusieurs fois jusqu'à atteindre presque la limite
		bankAccount.withdrawMoney(400.0);
		assertEquals(4600, bankAccount.getBalance(), 0.01);
		assertEquals(400, bankAccount.getAmountWithdrawn(), 0.01);

		// Then: un autre retrait qui dépasserait la limite échoue
		boolean result = bankAccount.withdrawMoney(350.0);
		assertFalse("Le retrait ne devrait pas réussir (limite cumulée dépassée)", result);
		assertEquals(4600, bankAccount.getBalance(), 0.01);
		assertEquals(400, bankAccount.getAmountWithdrawn(), 0.01);
	}

	@Test
	public void test_withdraw_at_exact_cumulative_limit() {
		// Given: un compte avec limite de 700
		bank.addAccount(bankAccount, assignAccountNumber);

		// When: on retire plusieurs fois pour atteindre presque la limite
		bankAccount.withdrawMoney(400.0);
		assertEquals(4600, bankAccount.getBalance(), 0.01);

		// When: on retire le montant restant autorisé (juste sous la limite)
		boolean result = bankAccount.withdrawMoney(299.0);

		// Then: le retrait réussit (total = 699 < 700)
		assertTrue("Le retrait devrait réussir", result);
		assertEquals(4301, bankAccount.getBalance(), 0.01);
		assertEquals(699, bankAccount.getAmountWithdrawn(), 0.01);
	}
